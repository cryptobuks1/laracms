@php
    use App\http\Controllers\Ninex\Ninex;
@endphp
@extends('admin.app')
...
<div class="content_wrapper">
	<div class="page_title">
		...
		<a class="btn btn-primary btn-sm" id="add_new_media_button" href="javascript:void(0);">Tải lên</a>
	</div>
						...
						<div class="row upload-panel-files" id="upload_image_preview">
							...
							<div class="col-lg-1 col-md-2 col-sm-3 col-xs-3 upload_file_preview">
								<div class="media_file" media-id="{{ $value->media_id }}" title="{{ $value->media_name . '.' . $value->media_extension }}">
									<div class="media_file_selected"><i class="dashicons dashicons-yes"></i></div>
									<div class="img_wrapper">
										<div class="img_show">
											<div class="img_thumbnail">
												<div class="img_centered">
													<img src="{{ Ninex::media($value->media_source, false) }}" class="{{ $value->media_style }}" alt="{{ $value->media_alt }}" draggable="false">
												</div>
											</div>
										</div>
									</div>
							...
							@endforeach
							@else
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="media_no_result">Không có dữ liệu.</div></div>
							@endif
...
<style>
	@php
	include_once(public_path('/contents/css/media.css'));
	@endphp
</style>
@endpush
@push('js')
<script type="text/javascript">
	@php
	include_once(public_path('/contents/js/media.min.js'));
	@endphp
</script>
<script type="text/javascript">
	...
	function media_filter(media_type, media_search, media_date){
		...
		$.ajax({
			...
			if(data.length > 0){
				...
				$('.media_show .row').html('<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="media_no_result">Không có dữ liệu.</div></div>');
			}
		}).fail(function(){
			alert('An error has occurred!');
		});
